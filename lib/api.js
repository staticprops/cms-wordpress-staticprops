const API_URL = "https://wp.staticprops.com/demo/graphql";
...
const convertBase64 = async (url, mimeType) => {
  // Proof of Concept: extra processing at build time, could also expand to add webp srcSet for example.
  ...
  const resizedImageBuf = await sharp(Buffer.from(buffer))
    .resize(24)
    .toBuffer();
